      ...
      {items.map((item) => (
        ...
        <div className={classes.itemWrapper} key={item.id}>
          ...
          <div className={classes.cardInfo}>
            <h3 className={classes.title}>{item.type} {item.model}</h3>
            ...
            <div className={classes.techParameters}>
              ...
              <li>Количество фотоэлементов в модуле, шт: {item.number_of_panels_in_set}</li>
              <li>Высота, мм: {item.height}</li>
              <li>Ширина, мм:{item.width}</li>
              <li>Глубина, мм: {item.depth_mm}</li>
              <li>Вес, кг: {item.weight_kg}</li>
              <li>Страна производитель: {item.country_of_registration_of_brand}</li>
            </div>
          </div>
        </div>
      ))}
